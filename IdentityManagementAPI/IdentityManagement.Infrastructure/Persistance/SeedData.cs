...
namespace IdentityManagement.Infrastructure.Persistance
{
    public class SeedData
    {
        public static void EnsureSeedData(IServiceProvider provider)
        {
            ...
            {
                var ıdentityResources = new List<IdentityResource>();
                configuration.GetSection("IdentityServer:IdentityResources").Bind(ıdentityResources);
                foreach (var ıdentityResource in ıdentityResources)
                    context.IdentityResources.Add(ıdentityResource.ToEntity());
                context.SaveChanges();
            }
